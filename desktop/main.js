...
async function createWindow() {
  ...
  process.env.AGENT_ROOM_RUNTIME_DIR = app.getPath("userData");
...
async function startDesktop() {
  ...
  app.setAppUserModelId("com.mohamedadel.agentroom");
...
if (!handleSquirrelEvent(squirrelEvent)) startDesktop().catch(async (error) => {
  ...
  try {
    dialog.showErrorBox("Agent Room couldn't start", `The local service failed to start (${error?.code || "startup_error"}). Check the application log and try again.`);
  } catch {}
  app.exit(1);
});
