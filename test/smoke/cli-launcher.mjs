// Smoke-tests the `agent-room` CLI launcher (bin/agent-room.mjs) as a real subprocess — not by importing
// the server directly like source-server.mjs — so the bin's own behavior is covered: it boots the
// server, serves the app, and honors AGENT_ROOM_RUNTIME_DIR for the data directory. This guards the
...
const bin = path.join(root, "bin", "agent-room.mjs");
const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), "agent-room-cli-smoke-"));

let child;
try {
  child = spawn(process.execPath, [bin], {
    env: { ...process.env, AGENT_ROOM_RUNTIME_DIR: runtimeDir, NO_OPEN: "1", PORT: "0" },
    stdio: ["ignore", "pipe", "pipe"],
  });

  // The launcher boots server/index.js, which prints "Agent Room is running at http://127.0.0.1:<port>".
  ...
  assert.match(await landing.text(), /Agent Room/);
