...
test("scratch setup failure keeps the original control and completes conservatively", async (t) => {
  const runtimeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "agent-room-repair-setup-"));
  const previousRuntimeRoot = process.env.AGENT_ROOM_RUNTIME_DIR;
  process.env.AGENT_ROOM_RUNTIME_DIR = runtimeRoot;
  ...
  try {
    ...
    if (previousRuntimeRoot === undefined) delete process.env.AGENT_ROOM_RUNTIME_DIR;
    else process.env.AGENT_ROOM_RUNTIME_DIR = previousRuntimeRoot;
