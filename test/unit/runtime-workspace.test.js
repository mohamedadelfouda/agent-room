...
test("a fresh desktop runtime creates its scratch workspace on first use", async () => {
  const runtime = await fs.mkdtemp(path.join(os.tmpdir(), "agent-room-runtime-"));
  const previous = process.env.AGENT_ROOM_RUNTIME_DIR;
  try {
    ...
    if (previous === undefined) delete process.env.AGENT_ROOM_RUNTIME_DIR;
    else process.env.AGENT_ROOM_RUNTIME_DIR = previous;
