...
test("logger rotates within a bounded file count", () => {
  const runtime = mkdtempSync(join(tmpdir(), "agent-room-log-rotate-"));
  try {
    ...
    `, { AGENT_ROOM_RUNTIME_DIR: runtime, AGENT_ROOM_LOG_MAX_BYTES: "1024" });
...
test("logger exposes write failures without crashing the process", () => {
  const dir = mkdtempSync(join(tmpdir(), "agent-room-log-fault-"));
  ...
  try {
    const state = childState(`
      ...
      process.stdout.write("STATE:" + JSON.stringify(loggerHealth()));
    `, { AGENT_ROOM_RUNTIME_DIR: invalidRuntime });
