...
const cookie = `agentRoomToken=${TOKEN}`;
...
test("checkApiAuth accepts a valid token via cookie or header", () => {
  ...
  assert.ok(checkApiAuth({ method: "GET", headers: { "x-agent-room-token": TOKEN } }, PORT).ok);
});

test("checkApiAuth rejects a wrong token with 401 (no crash on length mismatch)", () => {
  const r = checkApiAuth({ method: "GET", headers: { cookie: "agentRoomToken=deadbeef" } }, PORT);
  assert.equal(r.ok, false);
  assert.equal(r.status, 401);
});
