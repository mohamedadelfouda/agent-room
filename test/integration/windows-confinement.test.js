...
test("Windows exec confinement blocks host reads + network, allows the granted clone", async (t) => {
  ...
  try {
    const result = await runProcess({
      ...
      cwd: clone,
      containTree: true,
      windowsConfinement: { containerName: "AgentRoom.ConfineTest", grants: [clone] },
      timeoutMs: 60000,
    });
    ...
    await rm(join(homedir(), "AppData", "Local", "Packages", "AgentRoom.ConfineTest"), { recursive: true, force: true }).catch(() => {});
  }
});
...
test("Windows exec confinement fails closed when a granted path is invalid", async (t) => {
  ...
  const result = await runProcess({
    ...
    windowsConfinement: { containerName: "AgentRoom.ConfineTest", grants: ["C:\\agentroom-nonexistent-confine-xyz"] },
    timeoutMs: 30000,
  });
