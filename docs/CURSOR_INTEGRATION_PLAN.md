...
**الحكم الصحيح:** Claude **بيقدر ينفّذ** وعنده Bash sandbox على ماك/Linux/WSL2، **لكنه غير مؤهَّل حالياً كمنفّذ آمن داخل Agent Room** لأن:
...
3. **Agent Room لم يدمج/يختبر** الـ strict sandbox settings.

**فالقرار (review-only) صح — السبب اتغيّر من "مفيش sandbox" لـ:**
> *"Agent Room لم يدمج ويختبر strict Claude sandbox، وهو غير متاح على Windows native، وعزل Edit/Write يحتاج تحققاً منفصلاً."*
...
**⚠️ sandbox بتاع Cursor** حقيقي لأوامر الـshell، بس التوثيق بيوضّح إنه **مش boundary كامل** وإن بعض الأوامر ممكن ماتشتغلش داخله، و**بيخص terminal commands أساساً مش كل أدوات تعديل الملفات** (زي Claude). → **clone Agent Room المؤقت يفضل ضروري** حتى مع sandbox بتاع Cursor:
```
Agent Room clone   = يحمي الريبو الحقيقي من تعديل الملفات
...
- Claude داخل Agent Room مضبوط review-only **عمداً** (صحيح لبيئة Windows native).
...
- **معيار النجاح:** لا تغييرات في المشروع/الأب/Home · أي ملفات تشغيلية أُنشئت **فقط** داخل مجلدات Agent Room المؤقتة المسموح بها.
...
Cursor بوابة موديلات → بيعقّد المقارنة. سجّل لكل استدعاء: `provider=cursor, requestedModel, reportedModel, usage, duration, toolCalls, sandboxStatus`. **ومتقارنش "Claude direct vs Cursor GPT" وتستنتج إن الفرق من Agent Room** (الموديل والـ harness اتغيّروا معاً). اعمل مقارنات منفصلة: `Claude direct vs Cursor Sonnet` · `Codex direct vs Cursor GPT` · `2-provider vs 3-provider`.
...
- **أهم جملتين لازم تتكتبوا:** (1) *"Claude has a built-in Bash sandbox on macOS/Linux/WSL2, but remains unqualified for execution in Agent Room — especially on native Windows."* (2) *"Cursor's Windows Node entry point requires a provider-bound trusted launch descriptor; allowing generic `node.exe` is not acceptable."*
