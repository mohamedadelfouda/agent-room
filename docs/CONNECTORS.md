...
GitHub readiness checks both the native `gh` executable and its authentication state. Gmail is experimental: `AGENT_ROOM_GMAIL_ACCESS_TOKEN` is a short-lived access token and Agent Room does not refresh it, so an expired token must be replaced. Supabase configuration displays only the configured host; use a least-privilege key and enforce Row Level Security policies for the tables it can reach.
