...
// SECURITY.md).
const WINDOWS_EXEC_CONTAINER = "AgentRoom.CodexExec";
...
// "danger-full-access" — genuinely unsandboxed as far as Codex is concerned. Agent Room therefore refuses
...
export function windowsExecOptIn(env = process.env) {
  ...
  const appcontainer = on(env.AGENT_ROOM_WINDOWS_EXEC_APPCONTAINER);
  const unsandboxed = on(env.AGENT_ROOM_ALLOW_UNSANDBOXED_WINDOWS_EXEC);
...
export async function runCodex({ prompt, config, cwd, onEvent, registerChild }) {
  ...
  //   AGENT_ROOM_WINDOWS_EXEC_APPCONTAINER — run inside a Windows AppContainer that denies, by default,
  ...
  //   AGENT_ROOM_ALLOW_UNSANDBOXED_WINDOWS_EXEC — run with NO OS confinement (danger-full-access on the
  ...
  if (sandbox === null) {
    throw new Error(
      ...
      "AGENT_ROOM_WINDOWS_EXEC_APPCONTAINER=1 to run it inside an OS AppContainer (filesystem and network " +
      "denied except the disposable workspace), or set AGENT_ROOM_ALLOW_UNSANDBOXED_WINDOWS_EXEC=1 to allow " +
  ...
  try {
    ...
    if (processResult.windowsConfinementFailed) {
      const error = new Error(
        ...
        "set AGENT_ROOM_ALLOW_UNSANDBOXED_WINDOWS_EXEC=1 to accept a fully unsandboxed run.",
      );
