...
// to the container SID are reachable) and the network (no internetClient capability). Agent Room grants
...
public static class AgentRoomConfine {
...
if ($payload.confinement) {
  $sid = [IntPtr]::Zero
  try {
    $sid = [AgentRoomConfine]::EnsureContainer([string]$payload.confinement.containerName)
    $sidStr = [AgentRoomConfine]::SidString($sid)
    ...
    $exit = [AgentRoomConfine]::Launch($exe, $cmdline, $cwd, $sid)
    exit $exit
  } catch {
    # Fail closed: name the failure with a stable marker the server recognizes, and never fall back to
    # an unconfined launch.
    [Console]::Error.WriteLine("AGENTROOM_CONFINEMENT_SETUP_FAILED: " + $_.Exception.Message)
    exit 8086
  } finally {
    if ($sid -ne [IntPtr]::Zero) { [AgentRoomConfine]::ReleaseSid($sid) }
  }
}
