...
// App-level "update available" check (SD-4): compares Agent Room's own version to the latest published
...
const REPO = "mohamedadelfouda/agent-room";
...
// Agent Room's own version, read once at import from package.json.
...
export async function fetchLatestFromGitHub() {
  const response = await fetch(RELEASES_URL, {
    ...
    redirect: "error",
    headers: { accept: "application/vnd.github+json", "user-agent": "agent-room" },
  });
