...
function launch(sessionId, capability) {
  ...
  return {
    ...
    args: [serverPath],
    env: {
      AGENT_ROOM_SESSION_ID: id,
      AGENT_ROOM_MCP_BRIDGE_URL: bridgeUrl,
      AGENT_ROOM_MCP_BRIDGE_TOKEN: token,
      AGENT_ROOM_MCP_CAPABILITY: capability,
...
export function claudeMcpLaunch(sessionId, capability = "") {
  ...
  const mcpServers = config ? { agent_room: { type: "stdio", ...config } } : {};
  if (config) delete mcpServers.agent_room.token;
  return {
