...
function acceptedRef(commitSha) {
  ...
  return `refs/agent-room/accepted/${commitSha}`;
}
...
export async function listAcceptedRefs(projectPath) {
  const { stdout } = await git(["for-each-ref", "--format=%(refname)", "refs/agent-room/accepted/"], projectPath);
  return stdout.split(/\r?\n/).filter((ref) => /^refs\/agent-room\/accepted\/[0-9a-f]{40,64}$/i.test(ref));
}
...
async function indexPaths(projectPath) {
  ...
  return { indexPath, lockPath: `${indexPath}.lock`, intentPath: `${indexPath}.lock.agent-room-intent` };
}
...
export async function recoverAgentRoomIndexLock(projectPath) {
  ...
  const markerOwned = marker?.agentRoom === true && (!intent?.nonce || marker.nonce === intent.nonce);
...
export async function mergeBranch(projectPath, worktree, commitSha, ref = acceptedRef(commitSha)) {
  ...
  const { indexPath, lockPath, intentPath } = await indexPaths(projectPath);
  const temporaryIndex = `${indexPath}.agent-room-${crypto.randomUUID()}`;
  ...
  try {
    try {
      await fs.writeFile(lockPath, JSON.stringify({ agentRoom: true, nonce: lockNonce }), { flag: "wx", mode: 0o600 });
      ownsLock = true;
      ...
      await writeIntent(intentPath, { agentRoom: true, nonce: lockNonce, phase: "locked", lockIdentity });
    }
    catch (error) {
