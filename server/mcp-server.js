  ...
  if (request.method === "initialize") {
    return response(request.id, {
      ...
      instructions: capability === "project"
        ...
        : "Connector reads run only after the user enables that connector for this Agent Room session. Every state-changing tool creates a pending proposal; it never performs the external action. The user must approve that proposal in Agent Room before execution.",
    });
  }
...
async function run() {
  const sessionId = process.env.AGENT_ROOM_SESSION_ID;
  if (!sessionId) throw new Error("AGENT_ROOM_SESSION_ID is required");
  const bridgeUrl = process.env.AGENT_ROOM_MCP_BRIDGE_URL;
  const bridgeToken = process.env.AGENT_ROOM_MCP_BRIDGE_TOKEN;
  const capability = process.env.AGENT_ROOM_MCP_CAPABILITY;
  if (!bridgeUrl || !bridgeToken || !["project", "connectors"].includes(capability)) throw new Error("Agent Room MCP bridge is not configured");
  ...
  const handleLine = async (line) => {
    ...
    try {
      const bridgeResponse = await fetch(bridgeUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Agent-Room-MCP-Token": bridgeToken },
