...
export async function projectSnapshot(projectPath) {
  ...
  return [
    `--- ATTACHED PROJECT (read-only) ---`,
    `Path: ${projectPath}`,
    branch ? `Git: ${branch} @ ${head || "?"}${dirty ? ` — ${dirty} uncommitted change(s)` : " — clean"}` : `(not a git repository)`,
    tree ? `Top level: ${tree}` : "",
    `You can read any file here with the Read / Grep / Glob tools to ground your answer in the real code. Read only — never modify files or run commands.`,
    `------------------------------------`,
  ].filter(Boolean).join("\n");
}
